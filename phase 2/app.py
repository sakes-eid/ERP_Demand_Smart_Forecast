...
EXPECTED_OUTPUTS = {
    ...
    "procurement_capability_context": OUTPUT_DIR / "phase2_procurement_capability_context.csv",
    "supplier_strategy_summary": OUTPUT_DIR / "phase2_supplier_strategy_summary.csv",
    "procurement_kpi_summary": OUTPUT_DIR / "phase2_procurement_kpi_summary.csv",
}
...
def _format_ratio_mean(df: pd.DataFrame, column: str) -> str:
    """Format a mean ratio as a percentage."""
    if df.empty or column not in df.columns:
        return "N/A"
...
def _kpi_value(kpis: pd.DataFrame, name: str, percent: bool = False) -> str:
    """Return a named KPI from the compact summary."""
    if kpis.empty or not {"kpi_name", "kpi_value"}.issubset(kpis.columns):
        return "N/A"
    rows = kpis[kpis["kpi_name"].astype(str).eq(name)]
    if rows.empty:
        return "N/A"
    value = pd.to_numeric(pd.Series([rows.iloc[0]["kpi_value"]]), errors="coerce").iloc[0]
    if pd.isna(value):
        return "N/A"
...
def render_overview(data: dict[str, pd.DataFrame]) -> None:
    ...
    kpis = data["procurement_kpi_summary"]
    ...
    metric_row(
        [
            ("Requirement Coverage", _kpi_value(kpis, "requirement_coverage_rate", percent=True)),
            ("Unallocated Rate", _kpi_value(kpis, "unallocated_requirement_rate", percent=True)),
            ("Weighted Capacity Utilization", _kpi_value(kpis, "average_supplier_capacity_utilization_rate", percent=True)),
            ("Weighted Cost / Usable Unit", _kpi_value(kpis, "average_total_procurement_cost_per_usable_unit")),
        ]
    )
...
def render_supplier_performance(data: dict[str, pd.DataFrame]) -> None:
    ...
    metric_row(
        [
            ...
            ("OTIF", _format_ratio_mean(df, "otif_rate")),
            ("Fill Rate", _format_ratio_mean(df, "supplier_fill_rate")),
        ]
    )
...
def render_supplier_sku_options(data: dict[str, pd.DataFrame]) -> None:
    ...
    capability = data["procurement_capability_context"].copy()
...
def render_procurement_recommendations(data: dict[str, pd.DataFrame]) -> None:
    ...
    strategy = data["supplier_strategy_summary"].copy()
    df = filter_by_multiselect(df, "sku_id")
    ...
    metric_row(
        [
            ...
            ("Avg Cost / Usable", f"{numeric_mean(df, 'total_procurement_cost_per_usable_unit'):.2f}"),
            ("Single Source Risk", numeric_count(strategy, "supplier_concentration_risk_status", "SINGLE_SOURCE")),
        ]
    )
...
def render_risk_cost_analysis(data: dict[str, pd.DataFrame]) -> None:
    ...
    capability = data["procurement_capability_context"].copy()
    kpis = data["procurement_kpi_summary"].copy()

    metric_row(
        [
            ("Weighted Cost / Usable", _kpi_value(kpis, "average_total_procurement_cost_per_usable_unit")),
            ("Requirement Coverage", _kpi_value(kpis, "requirement_coverage_rate", percent=True)),
            ("Weighted Capacity Utilization", _kpi_value(kpis, "average_supplier_capacity_utilization_rate", percent=True)),
            ("Top Supplier Share", _kpi_value(kpis, "average_top_supplier_allocation_share", percent=True)),
        ]
    )
    ...
    with col2:
        ...
        _histogram(capability, "total_procurement_cost_per_usable_unit", "Cost per Usable Unit Distribution")
...
def render_feasibility_review(data: dict[str, pd.DataFrame]) -> None:
    ...
    metric_row(
        [
            ("Avg Capacity Utilization", _format_ratio_mean(capability, "supplier_capacity_utilization_rate")),
            ("High Utilization Options", numeric_count(capability, "supplier_capacity_utilization_status", "HIGH")),
            ("Fully Used Options", numeric_count(capability, "supplier_capacity_utilization_status", "FULLY_USED")),
            ("Cost / Usable Rows", int(pd.to_numeric(capability.get("total_procurement_cost_per_usable_unit", pd.Series(dtype=float)), errors="coerce").notna().sum()) if not capability.empty else 0),
        ]
    )
