...
def _add_feasibility_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched["reference_usable_quantity"] = reference["reference_usable_quantity"]
    enriched["reference_quantity_source"] = reference["reference_quantity_source"]
...
def _reference_quantity(row: pd.Series) -> pd.Series:
    ...
    if pd.notna(p90) and p90 > 0:
        return pd.Series({"reference_usable_quantity": float(p90), "reference_quantity_source": "AVERAGE_P90_FORECAST"})
    if pd.notna(p50) and p50 > 0:
        return pd.Series({"reference_usable_quantity": float(p50), "reference_quantity_source": "AVERAGE_P50_FORECAST"})
    return pd.Series({"reference_usable_quantity": float(row["moq"]), "reference_quantity_source": "MOQ_FALLBACK"})
...
def _add_demand_adjusted_risk(df: pd.DataFrame) -> pd.DataFrame:
    ...
    enriched = df.copy()
    demand_risk_factor = _demand_risk_factor(enriched)
    enriched["demand_adjusted_procurement_risk_score"] = (
        enriched["procurement_risk_score"] + demand_risk_factor
    ).clip(0, 1)
...
def _demand_risk_factor(df: pd.DataFrame) -> pd.Series:
    """Return a demand risk factor between 0 and 0.20."""
    confidence_penalty = (1 - pd.to_numeric(df["champion_confidence_score"], errors="coerce").fillna(0.5)).clip(0, 1)
    forecast_risk_penalty = (
        df["champion_risk_level"].astype(str).eq("HIGH_RISK").astype(float) * 0.08
        + df["dominant_forecast_risk_level"].astype(str).eq("HIGH_RISK").astype(float) * 0.06
        + df["champion_risk_level"].astype(str).eq("MEDIUM_RISK").astype(float) * 0.03
    )
    variability_penalty = (pd.to_numeric(df["coefficient_of_variation"], errors="coerce").fillna(0) / 2).clip(0, 1) * 0.04
    intermittency_penalty = pd.to_numeric(df["zero_demand_ratio"], errors="coerce").fillna(0).clip(0, 1) * 0.04
    low_data_penalty = df["data_sufficiency_class"].astype(str).str.lower().eq("low").astype(float) * 0.04
    confidence_penalty = confidence_penalty * 0.06
    return (confidence_penalty + forecast_risk_penalty + variability_penalty + intermittency_penalty + low_data_penalty).clip(0, 0.20)
